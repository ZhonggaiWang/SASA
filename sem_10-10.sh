...
for t in 1; do
  python -m torch.distributed.launch --nproc_per_node=2 --master_port=${master_port} scripts/dist_train_voc_seg_neg.py --step ${t} --max_iters 12000 --lr 2e-5 --task ${task} --work_dir output_voc
done
